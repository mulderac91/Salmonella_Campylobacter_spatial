...
# Find the pc4's that are duplicated as example (fill in the record number as given by dubl:
...
salmonella_pc4 <- distinct(salmonella_pc4)
...
# Drop S. Typhi, Paratyphi A, Paratyphi B, Paratyphi B variatie Java, 
# they are thypoidal and are not causing gastro intestinal complaints, we are only looking at NTS
...
# For selection: Typhimurium, remove other serotypes/change description
...
strings <- c("Enteritidis", "Typhimurium")
salmonella_pc4 <- salmonella_pc4 %>%
  filter(str_detect(`serotype omschrijving`, paste(strings, collapse = "|")))

# Remove individual cases linked to food sources
# Load dataset with cases linked to food sources
...
salmonella_pc4 <- salmonella_pc4[!is.na(salmonella_pc4$age),]
...
#remove buitenland isolates
salmonella_pc4 <- salmonella_pc4 %>% filter(is.na(Buitenland))
#Make ? in sex NA and remove NA's
sum(is.na(salmonella_pc4))
salmonella_pc4$sex[salmonella_pc4$sex == '?'] <- NA
salmonella_pc4 <- salmonella_pc4[!is.na(salmonella_pc4$sex), ]

#filter on PC4 > 0 and < 9999
salmonella_pc4 <- salmonella_pc4[(salmonella_pc4$pc4 > 0 & salmonella_pc4$pc4 < 9999), ]
# remove cases from which pc4s are unknown
salmonella_pc4 <- salmonella_pc4[!is.na(salmonella_pc4$pc4), ]
...
# S. Enteritidis:
salmonella_pc4 <- salmonella_pc4 %>%
  filter(str_detect(`serotype omschrijving`, "Enteritidis"))

# S. Typhimurium:
# salmonella_pc4 <- salmonella_pc4 %>%
# filter(str_detect(`serotype omschrijving`, "Typhimurium"))
...
# -----------------------Dierdata koppeling moet per jaar!!----------------------------------------------------------------------------------------------------
...
# check calculations of weighted animal number
# plot(animal.hex.sf.2007["Pluimvee_totaal"], breaks = "fisher")
# plot(animal.hex.sf.2007["Rundvee_totaal"], breaks = "fisher")
...
# Save dataset
#saveRDS(hex.agg.sf, file = ".../Results/Maps/Figure 1/Rds files/incidence_SE.Rds")

# Save animal datasets
# saveRDS(animal.hex.sf.2012, file = ".../Results/Maps/Figure S1/Rds files/animal.hex.sf.2012.rds")
# saveRDS(animal.hex.sf.2015, file = ".../Results/Maps/Figure S1/Rds files/animal.hex.sf.2015.Rds")
# saveRDS(animal.hex.sf.2018, file = ".../Results/Maps/Figure S1/Rds files/animal.hex.sf.2018.Rds")
...
#   drop_na()
# export
#   st_write(hex.sf, ".../Results/hex.sf.covered.salmonella.shp")
#  saveRDS(hex.sf, file = ".../Results/Maps/Figure 1/Rds files/covered_hex_salmonella.Rds")
